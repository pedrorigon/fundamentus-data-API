...
async def postgres_lock_schema(connection: Any, name: str) -> None:
    """Serialize schema creation across replicas starting at the same time.

    ``CREATE TABLE IF NOT EXISTS`` still races when two connections create the
    same table concurrently; a transaction-scoped advisory lock lets the second
    replica wait for the first one instead of failing its startup.
    """
    await connection.execute("SELECT pg_advisory_xact_lock(hashtext(%s))", (name,))
